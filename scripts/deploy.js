...
async function main() {
  ...
  console.log("Transaction Hash: ", `https://explorer-evm.testnet.swisstronik.com/address/${swisstronik.target}`);
  
  const upgradedSwisstronik = await upgrades.deployProxy(Swisstronik, ['Hello Swisstronik!!'], { kind: 'transparent' });
  ...
  console.log("Transaction Hash: ", `https://explorer-evm.testnet.swisstronik.com/address/${upgradedSwisstronik.target}`);
}
main()
  .then(() => process.exit(0))
  .catch((error) => {
